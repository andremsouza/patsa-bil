"""This module contains the DINODataset to load images and labels for DINO training."""
...
class DINODataset(SamDataset):
    """Class to load images and labels for DINO training."""
    ...
    def __init__(
        ...
        Args:
            ...
            others_class_list (Optional[list[str]], optional): List of other class names.
                Defaults to None.
            background_class (bool, optional): Whether to include a background class.
                Defaults to False.
            transform (Optional[Callable], optional): Transform to apply to the images.
                Defaults to None.
            target_transform (Optional[Callable], optional): Transform to apply to the targets.
                Defaults to None.
            target_labels (bool, optional): Whether to include labels in the targets.
                Defaults to False.
            target_masks (bool, optional): Whether to include masks in the targets.
                Defaults to False.
            target_boxes (bool, optional): Whether to include bounding boxes in the targets.
                Defaults to False.
        ...
        self.dataframe_dino = self._build_detr_dataframe()

    def _build_detr_dataframe(self) -> pd.DataFrame:
        # Create datsaet to iterate on image/label files
        dataframe_dino = self.dataframe.copy()
        dataframe_dino = dataframe_dino.loc[:, ["image_file", "label_file"]]
        # Remove duplicates
        dataframe_dino.drop_duplicates(inplace=True)

        return dataframe_dino

    def _load_label_detr(self, label_path: str) -> dict:
    ...
    def __getitem__(self, idx: int):
        img = torchvision.io.read_image(self.dataframe_dino.iloc[idx]["image_file"])
        w_orig, h_orig = img.size()[-2:]
        ...
        if self.list_labels_paths:
            labels = self._load_label_detr(self.dataframe_dino.iloc[idx]["label_file"])
            ...
            labels["iscrowd"] = torch.tensor(
                [0] * len(labels["labels"]), dtype=torch.uint8
            )
            labels["image_id"] = torch.tensor([idx])
            labels["orig_size"] = torch.as_tensor([int(h_orig), int(w_orig)])
            labels["size"] = torch.as_tensor([int(h), int(w)])
            labels_orig = labels.copy()
            if self.target_transform:
                labels = self.target_transform(labels)
            return img, labels, img_orig, labels_orig
        else:
            return img, img_orig

    def __len__(self) -> int:
        return len(self.dataframe_dino)
