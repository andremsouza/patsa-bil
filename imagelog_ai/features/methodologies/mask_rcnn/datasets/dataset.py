...
class MaskRCNNDataset(SamDataset):
    """Class to load images and labels for Mask R-CNN training."""
    ...
    def __init__(
        ...
        self.dataframe_maskrcnn = self._build_maskrcnn_dataframe()

    def _build_maskrcnn_dataframe(self) -> pd.DataFrame:
        # Create datsaet to iterate on image/label files
        dataframe_maskrcnn = self.dataframe.copy()
        dataframe_maskrcnn = dataframe_maskrcnn.loc[:, ["image_file", "label_file"]]
        # Remove duplicates
        dataframe_maskrcnn.drop_duplicates(inplace=True)

        return dataframe_maskrcnn

    def _load_label_maskrcnn(self, label_path: str) -> dict:
    ...
    def __getitem__(self, idx: int):
        img = torchvision.io.read_image(self.dataframe_maskrcnn.iloc[idx]["image_file"])
        ...
        if self.list_labels_paths:
            labels = self._load_label_maskrcnn(
                self.dataframe_maskrcnn.iloc[idx]["label_file"]
            )
    ...
    def __len__(self) -> int:
        return len(self.dataframe_maskrcnn)
