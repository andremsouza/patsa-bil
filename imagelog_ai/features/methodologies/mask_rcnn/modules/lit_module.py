"""This module contains the LitMaskRCNN class, which is a PyTorch Lightning Module for Mask R-CNN.
"""
...
class LitMaskRCNN(pl.LightningModule):
    ...
    def __init__(
        ...
        """Constructor for the LitMaskRCNN class.
    ...
    def _build_network(self):
        ...
        network: Sequential
            Returns a Sequential with the layers of the network that will be used for training,
                testing and predict.
        """
        network = maskrcnn_resnet50_fpn_v2(
            weights=MaskRCNN_ResNet50_FPN_V2_Weights.DEFAULT,
            trainable_backbone_layers=5 - self.num_frozen_blocks,
        )
        box_in_features = network.roi_heads.box_predictor.cls_score.in_features
        network.roi_heads.box_predictor = FastRCNNPredictor(
            box_in_features, self.num_classes
        )
        mask_in_features = network.roi_heads.mask_predictor.conv5_mask.in_channels
        network.roi_heads.mask_predictor = MaskRCNNPredictor(
            mask_in_features, mask_in_features, self.num_classes
        )
