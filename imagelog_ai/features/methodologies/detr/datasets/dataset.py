"""This module contains the DETRDataset to load images and labels for DETR training."""
...
class DETRDataset(SamDataset):
    """Class to load images and labels for DETR training."""
    ...
    def __init__(
        ...
        self.dataframe_detr = self._build_detr_dataframe()

    def _build_detr_dataframe(self) -> pd.DataFrame:
        # Create datsaet to iterate on image/label files
        dataframe_detr = self.dataframe.copy()
        dataframe_detr = dataframe_detr.loc[:, ["image_file", "label_file"]]
        # Remove duplicates
        dataframe_detr.drop_duplicates(inplace=True)

        return dataframe_detr
    ...
    def __getitem__(self, idx: int):
        img = torchvision.io.read_image(self.dataframe_detr.iloc[idx]["image_file"])
        ...
        if self.list_labels_paths:
            labels = self._load_label_detr(self.dataframe_detr.iloc[idx]["label_file"])
    ...
    def __len__(self) -> int:
        return len(self.dataframe_detr)
