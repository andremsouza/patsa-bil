...
from imagelog_ai.features.methodologies.detr.utils.box_ops import (
    box_cxcywh_to_xyxy,
    generalized_box_iou,
)
...
class HungarianMatcher(nn.Module):
    ...
    there are more predictions than targets. In this case, we do a 1-to-1 matching
        of the best predictions, while the others are un-matched (and thus treated as non-objects).
    """
    ...
    def __init__(
        ...
        Params:
            cost_class: This is the relative weight of the classification error in
                the matching cost
            ...
            targets: This is a list of targets (len(targets) = batch_size),
                where each target is a dict containing:
                 "labels": Tensor of dim [num_target_boxes] (where num_target_boxes is
                    the number of ground-truth objects in the target) containing the class labels
...
def build_matcher(args):
    """Builds and returns a HungarianMatcher object with the specified cost parameters.
    Args:
    ...
    Returns:
        HungarianMatcher: An instance of HungarianMatcher initialized
            with the provided cost parameters.
    """
